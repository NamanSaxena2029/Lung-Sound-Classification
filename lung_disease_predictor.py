...
if uploaded_file is not None:
    ...
    try:
        ...
        st.table({"Disease": list(prob_dict.keys()), "Probability (%)": [f"{v*100:.2f}%" for v in prob_dict.values()], "Confidence": ["High" if i==predicted_class else "Low" for i in range(len(index_to_label))]})
        # Recommendations
        st.markdown("#### Recommendations")
